...
$(document).ready(function() {
	$("#email_address").change(function() {
		if($('#email_address').val().length > 0)
		{
			...
			$.ajax({
				type: 'POST',
				url: base_url+'user/NonLoged/email_check',
				data: { 'email': email},
				success: function(res) {
					if(res == 1) {
						$('#exitemail').show();
						$('#exitemail').addClass('help-block');
						$('#exitemail').addClass('has-error');
						$("#email_address").addClass("has-error");
						$("#email_address").removeClass("has-success");
						$("#submit_button").attr("disabled", true);
					} else if(res == 0) {
						$("#exitemail").hide();
						$('#exitemail').removeClass('help-block');
						$('#exitemail').removeClass('has-error');
						$("#submit_button").attr("disabled", false);
						$("#email_address").addClass( "" );
					}
				}
			});
		}
		...
		if ($.validator) {
			$('.login-form').validate({
				...
				/*submitHandler: function (form) {
					window.location.href = "/hubway/index.php?r=user/auth/login";
